...
[Colab Notebook（可直接运行）](https://colab.research.google.com/drive/1e2gmakoP7h5DNnRF1BhEyzy9YMRVdL5c?usp=sharing)

# qwen3.5 小模型在企业中的实际应用（2/4）：bi面板实时自动解读

本篇使用 **Google Colab** 完成训练与推理链路，GPU 使用 **T4**。

目标是实现 BI 数据解读的双模型路由：默认走 0.8B，低置信度或规则命中时回退到 2B。

## 模块目录

- 模块01：1. 安装依赖（含 swanlab）
- 模块02：1.1 重启运行时（必须）
- 模块03：2. 重启后检查版本
- 模块04：3. 检查 GPU
- 模块05：4. 生成数据
...
- 模块07：6. 准备测试数据
- 模块08：7. 模型懒加载
- 模块09：8. （可选）挂载 LoRA Adapter
- 模块10：9. 路由规则与质量评分
- 模块11：10. 推理函数（单模型）
- 模块12：11. 路由执行（0.8B主路+2B回退）
- 模块13：12. 单条测试
- 模块14：13. 批量路由+swanlab记录
- 模块15：14. 保存结果
